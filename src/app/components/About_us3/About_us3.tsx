    ...
    <section className="bg-[#fff8f0] py-16 md:py-24">
      ...
      <div className="mx-auto mt-10 max-w-[1200px] px-4">
        <div className="relative overflow-hidden rounded-[48px] md:rounded-[56px]">
          {/* shorter height on mobile to keep composition tight */}
          ...
          <div className="pointer-events-none absolute inset-0 grid place-items-center px-3">
            {/* FIXED 3 columns on all sizes (no scroll) */}
            <div
              className="
                grid w-full max-w-5xl grid-cols-3 gap-3 sm:gap-6
              "
            >
              {AWARDS.map((a) => (
                <div
                  key={a.title}
                  className="
                    pointer-events-auto rounded-[20px] md:rounded-[28px]
                    bg-white/55 backdrop-blur-md ring-1 ring-white/60
                    shadow-[0_10px_30px_rgba(0,0,0,0.15)]
                    px-3 py-4 md:px-8 md:py-10
                  "
                >
                  {/* logo */}
                  <div className="mx-auto grid h-16 w-16 md:h-40 md:w-40 place-items-center rounded-2xl bg-white/70 ring-1 ring-white/80 shadow-sm">
                    <Image
                      src={a.logo}
                      alt={a.alt}
                      width={160}
                  ...
                  <div className="mt-3 md:mt-6 text-center">
                    <p className="font-serif text-[12px] leading-snug md:text-[18px] text-[#2e2417]">
                      {a.title}
                    </p>
                    <p className="mt-1 text-sm md:text-lg font-semibold text-[#2e2417]">
                      {a.year}
                    </p>
                  </div>
                </div>
